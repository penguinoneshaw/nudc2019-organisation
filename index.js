...
const { individual, team, qualifiers } = require("./data/events");

const config = {
    ...require("./data/slides.json"),
    competitions: { ...individual, ...team, ...qualifiers },
};

let base_template_promise = fs
    .readFile("handlebars_templates/template.hbs", { encoding: "utf8" })
    .then((template) => hbs.compile(template));
let invoice_template_promise = fs
    .readFile("handlebars_templates/invoice.hbs", { encoding: "utf8" })
    .then((template) => hbs.compile(template));
let invoice_list_template_promise = fs
    .readFile("handlebars_templates/invoice_list.hbs", { encoding: "utf8" })
    .then((template) => hbs.compile(template));
let index_promise = fs
    .readFile("handlebars_templates/index.hbs", { encoding: "utf8" })
    .then((template) => hbs.compile(template));
let university_index_promise = fs
    .readFile("handlebars_templates/university_index.hbs", { encoding: "utf8" })
    .then((template) => hbs.compile(template));
let event_index_promise = fs
    .readFile("handlebars_templates/event_index.hbs", { encoding: "utf8" })
    .then((template) => hbs.compile(template));
const presentation_template = fs
    .readFile("framework/index.hbs", { encoding: "utf8" })
    .then((template) => hbs.compile(template));
const competitor_list_template = fs
    .readFile("handlebars_templates/competitor_list.hbs", { encoding: "utf8" })
    .then((template) => hbs.compile(template));

async function render_invoice(form_output) {
...
async function build_index() {
    const dir_listing = await fs.readdir("entries");
    const universities = await Promise.all(
        dir_listing.map((entry) =>
            fs.readFile(`entries/${entry}/json/full_save.json`)
        )
    ).then((entries) =>
        entries
            .map((entry) => JSON.parse(entry))
            .reduce((object, university) => {
                return {
                    ...object,
...
app.get("/invoices", async (req, res) => {
    ...
    const { universities } = await combined_universities;
    ...
    return Promise.all([
        base_template_promise,
        invoice_list_template_promise,
    ]).then((base_template) =>
        res.send(
            base_template[0]({
...
app.get(/university\/(.*)\/invoice$/, async (req, res) => {
    try {
      const { universities } = await combined_universities;
      const content = await render_invoice(
          universities[decodeURIComponent(req.params[0])]
      );
      const in_context = await base_template_promise.then((template) =>
          template(content)
      );
      res.set({
        "Content-Type": "text/html",
...
app.get(/competitors$/, async (req, res) => {
    try {
      const { full_list } = await combined_universities;
      const in_context = await competitor_list_template.then((template) =>
          template(full_list)
      );
      res.set({
        "Content-Type": "text/html",
...
app.get(/university\/(.*)$/, async (req, res) => {
    try {
      const { universities, full_list } = await combined_universities;
      res.set({
        "Content-Type": "text/html",
    });
      ...
      const ret = {
        ...
        couples: Object.values(full_list.couples)
            .filter((c) => c.university === university)
            .map((cpl) => {
                return {
                    ...cpl,
...
app.get(/event\/([a-z]+)$/, async (req, res) => {
    ...
    const event_couples = Object.values(full_list.couples)
        .filter((c) => c.events.includes(req.params[0]))
        .map((c) => {
            return {
                ...c,
...
function competitor_entry_category(competitor, offbeat = false, late = false) {
    ...
    let type = "";
    if (
        (competitor.events.length == 0 && competitor.offbeat && offbeat) ||
        ((competitor.events.includes("onarr") ||
            competitor.events.includes("oarr")) &&
            competitor.events.length == 1) ||
        (competitor.events.includes("onarr") &&
            competitor.events.includes("oarr") &&
            competitor.events.length == 2)
    ) {
        type = "offrnr";
    } else if (
        competitor.events.length == 0 &&
        !(competitor.offbeat && offbeat)
    ) {
        type = "spectator";
    } else {
      type = "full";
...
function generate_invoice_object(form_output, late = false) {
    const competitors_with_entrytype = Object.values(form_output.competitors).map(
        (competitor) => {
            return {
                ...competitor,
          entry_type: competitor_entry_category(
              competitor,
              form_output.offbeat,
              late
          ),
    ...
    const summary = Object.values(competitors_with_entrytype).reduce(
        (prev, next) => {
            return {
                ...prev,
                [next.entry_type]: {
    ...
    return {
      reference: `NUDC ${form_output.basic_information.university
          .split(/university|and(?!\w)|of(?!\w)/gi)
          .join("")}`
          .substr(0, 18)
          .trim(),
      ...
      total_entry: Object.values(summary).reduce(
          (prev, next) => prev + next.number * next.unit_cost,
          0
      ),
  };
}
...
hbs.registerHelper("couple-row", (context, opts) => {
    return Object.values(context)
        .sort((a, b) => (a.comp_couple_id > b.comp_couple_id ? 1 : -1))
        .reduce((prev, next) => {
            return (
                prev +
                opts.fn({
...
hbs.registerHelper("all-events-row", (context, opts) => {
    return Object.keys(individual)
        .map((event_id) => {
            const event_couples = Object.values(context.couples).filter((c) =>
                c.events.includes(event_id)
            );
            return opts.fn({
...
hbs.registerHelper("university-row", (context, opts) => {
    return Object.values(context).reduce((prev, entry) => {
      const competitors_length = Object.values(entry.competitors).filter(
          (competitor) => competitor.events.length || competitor.offbeat
      ).length;
      const couples_length = Object.values(entry.couples).filter(
          (couple) => couple.events.length
      ).length;
      return (
          prev +
          opts.fn({
              university: entry.basic_information.university,
          link: `university/${encodeURIComponent(
              entry.basic_information.university
          )}`,
          competitors_length,
          couples_length,
      })
...
function combine_full_competition_individual_lists(
    combined_entries_by_university
) {
    let result = {
        couples: [],
        competitors: {},
    ...
    result.couples
      ...
      .forEach((couple, i) => {
          if (
              result.couples_fixed.slice(-1)[0] &&
              result.couples_fixed.slice(-1)[0].university === couple.university
          ) {
              if (
                  result.couples_fixed[0] &&
                  result.couples_fixed[0].university === couple.university
              ) {
                  result.couples_fixed.splice(
                      result.couples_fixed.length / 4 +
                      (i % result.couples_fixed.length) / 2,
                      0,
                      couple
                  );
    ...
    result.couples_map = result.couples_fixed.reduce((prev, next, i) => {
      ...
      const comp_couple_id = BANNED_NUMBERS.includes(i + 1)
          ? result.couples.length + BANNED_NUMBERS.indexOf(i + 1) + 1
          : i + 1;
      return {
          ...prev,
          [comp_couple_id]: {
